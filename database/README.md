...
PostgreSQL 16 database for App application.
...
```bash
# Create backup
docker exec docker-nginx-react-frontend-admin-database-1 pg_dump -U App_user App_db > backup_$(date +%Y%m%d).sql

# Compressed backup
docker exec docker-nginx-react-frontend-admin-database-1 pg_dump -U App_user App_db | gzip > backup_$(date +%Y%m%d).sql.gz
```

### Restore
```bash
# Restore from backup
docker exec -i docker-nginx-react-frontend-admin-database-1 psql -U App_user App_db < backup.sql

# Restore from compressed backup
gunzip < backup.sql.gz | docker exec -i docker-nginx-react-frontend-admin-database-1 psql -U App_user App_db
```
...
docker exec -it docker-nginx-react-frontend-admin-database-1 psql -U App_user -d App_db

# Run SQL file
docker exec -i docker-nginx-react-frontend-admin-database-1 psql -U App_user -d App_db < query.sql
```
...
### Vacuum Database
```bash
docker exec docker-nginx-react-frontend-admin-database-1 vacuumdb -U App_user -d App_db -z
```
